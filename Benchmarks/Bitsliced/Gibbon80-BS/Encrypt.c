...
void crypto_aead_encrypt(
	...
	//Calculate tag (Actually the last 3 XORs are not needed, as we only need 80 bit for the tag... They could be removed, depending on which bits is used for the tag.
	for (int i = 0; i < 5; i++) {
		state[0][0] = XOR(state[0][0], key[0]);
		state[1][0] = XOR(state[1][0], key[1]);
		state[2][0] = XOR(state[2][0], key[2]);
...
int crypto_aead_decrypt(
	...
	while (progress < clen) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (progress < clen) {
				GCC_mm256_insert_epi64(state[i][0], data_u64[i], 0);
			}
		}
			...
			for (int i = 0; i < 5; i++) {
				...
				if (clen_last_blocksize >= clen_pad_progress + 8) {
					...
					if (clen_last_blocksize < clen_pad_progress) {
						...
						GCC_mm256_insert_epi64(dec_m[i], t, 0);
					}
				}
				clen_pad_progress += 8;
	...
	GCC_mm256_insert_epi8(state[1][0], 0x00, 15); //We only compare 80 bits, not 128.
	GCC_mm256_insert_epi8(state[1][0], 0x00, 14);
	GCC_mm256_insert_epi8(state[1][0], 0x00, 13);
	GCC_mm256_insert_epi8(state[1][0], 0x00, 12);
	GCC_mm256_insert_epi8(state[1][0], 0x00, 11);
	GCC_mm256_insert_epi8(state[1][0], 0x00, 10);
...
void init_bitslice_state(YMM *key, const u8 *n, const u8 *k, YMM(*state)[2]) {
	...
	key_bits[0] = (k[0] << 8) | (k[5] << 16) | (n[0] << 24);
	key_bits[1] = (k[1] << 8) | (k[6] << 16) | (n[1] << 24);
	key_bits[2] = (k[2] << 8) | (k[7] << 16) | (n[2] << 24);
	key_bits[3] = (k[3] << 8) | (k[8] << 16) | (n[3] << 24);
	key_bits[4] = (k[4] << 8) | (k[9] << 16) | (n[4] << 24);
